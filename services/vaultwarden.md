...
Vaultwarden is used as the self-hosted password manager and primary secret store for the lab.

It is reachable only over the private Tailscale network and is not exposed publicly.
...
\- `ADMIN\_TOKEN` set to a long random secret

\- `SIGNUPS\_ALLOWED=false` after initial bootstrap
...
\- Admin panel is available at `/admin` and protected by `ADMIN\_TOKEN`.
...
`/srv/edge-lab/volumes/vaultwarden/backup\_YYYY-MM-DD\_HHMMSS.tar.gz`
